...
export function bindControls(generateFn: () => void): void {
  ...
  btnRandom.addEventListener("click", () => {
    seedInput.value = String(Math.floor(Math.random() * 4294967295));
  });
...
export function readParams(): { seed: number; width: number; height: number; sea: number } {
  const seedInput = document.getElementById("seed-input") as HTMLInputElement;
  const widthInput = document.getElementById("width-input") as HTMLInputElement;
  const heightInput = document.getElementById("height-input") as HTMLInputElement;
  const seaSlider = document.getElementById("sea-slider") as HTMLInputElement;
...
export function updateWorldInfo(seed: number, width: number, height: number, sea: number): void {
  const panel = document.getElementById("world-info")!;
  panel.style.display = "";
  document.getElementById("info-seed")!.textContent = String(seed);
  document.getElementById("info-size")!.textContent = `${width} \u00d7 ${height}`;
  document.getElementById("info-tiles")!.textContent = (width * height).toLocaleString();
  document.getElementById("info-sea")!.textContent = sea.toFixed(2);
}
