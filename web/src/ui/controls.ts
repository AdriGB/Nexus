...
import { resizeCanvas } from "../renderer/world-renderer"; // FIX #7
import { updateTileInspector } from "./tile-inspector"; // FIX #4
...
export function bindControls(generateFn: () => void): void {
  ...
  window.addEventListener("keydown", (e) => {
    ...
    // FIX #4: Also clear the inspector panel
    if (e.key === "Escape") {
  ...
  // FIX #7: Resize only when window actually changes
