...
function syncEntityInspector(): void {
  ...
  grid.innerHTML = [
    ...
    `<span class="info-key">Life Stage</span><span class="info-val">${entity.life_stage}</span>`,
    `<span class="info-key">Stage Speed</span><span class="info-val">${(entity.stage_movement_factor * 100).toFixed(0)}%</span>`,
    `<span class="info-key">Caregiver</span><span class="info-val">${entity.caregiver_id ?? "—"}</span>`,
    `<span class="info-key" style="grid-column:1/-1;border-top:1px solid var(--border);margin-top:4px;padding-top:4px;">Personality</span>`,
    `<span class="info-key">Curiosity</span><span class="info-val">${(entity.personality.curiosity * 100).toFixed(0)}%</span>`,
    `<span class="info-key">Sociability</span><span class="info-val">${(entity.personality.sociability * 100).toFixed(0)}%</span>`,
    `<span class="info-key">Cooperativeness</span><span class="info-val">${(entity.personality.cooperativeness * 100).toFixed(0)}%</span>`,
    `<span class="info-key">Caution</span><span class="info-val">${(entity.personality.caution * 100).toFixed(0)}%</span>`,
    `<span class="info-key">Persistence</span><span class="info-val">${(entity.personality.persistence * 100).toFixed(0)}%</span>`,
